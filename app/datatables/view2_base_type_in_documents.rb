class View2BaseTypeInDocuments
  ...
  # constructor
  def initialize(view, document_coba)
    @view = view
    @document_coba = document_coba
  ...
  def fetch_documents
    documents = Document.left_outer_joins(:document_type, :sub_document_type, :document_reference, :document_classification, :sub2_document_type).where(sub2_document_type_id: params[:sub2_document_type_id]).select('*').order("documents.document_code ASC")
    documents = documents.paginate(page: page, per_page: per_page)
    if params[:document_coba].present?
      begin
        Date.strptime(params[:document_coba])
        documents = documents.where('documents.document_date = ?', "#{params[:document_coba]}")
      rescue ArgumentError
        # handle invalid date
        documents = documents.where('lower(documents.document_title) LIKE ? or lower(documents.document_code) LIKE ?', "%#{params[:document_coba].downcase}%", "%#{params[:document_coba].downcase}%")
      end
    end
    documents
  end
