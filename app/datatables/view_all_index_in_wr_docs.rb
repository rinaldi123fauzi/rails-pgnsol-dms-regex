class ViewAllIndexInWrDocs
  delegate :link_to, :view_show_wr_docs_path, :wr_doc_path, :wr_view_document_path,
  ...
  def fetch_documents
    documents = WrDoc.left_outer_joins(:sub_field, :field, :wr_doc_type, :sub2_field, :sub3_field).select('*').order("wr_docs.document_code asc, #{sort_column} #{sort_direction}, wr_docs.date DESC")
