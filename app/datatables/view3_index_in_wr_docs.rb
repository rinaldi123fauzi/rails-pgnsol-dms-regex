class View3IndexInWrDocs
  ...
  def fetch_documents
    documents = WrDoc.left_outer_joins(:sub_field, :field, :wr_doc_type, :sub2_field, :sub3_field).where('wr_docs.sub3_field_id = ?', params[:sub3_field_id]).select('*').order("wr_docs.document_code asc, #{sort_column} #{sort_direction}, wr_docs.date DESC")
